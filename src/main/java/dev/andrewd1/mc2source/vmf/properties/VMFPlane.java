...
public class VMFPlane extends VMFProperty<Triple<Vector, Vector, Vector>> {
    ...
    private static Vector getFirstPoint(BlockFace face) {
        return switch (face) {
            case UP -> new Vector(0, 1, 0);  // bad
            case DOWN -> new Vector(0, 0, 0);
            case NORTH -> new Vector(0, 0, 0);  // bad
            case SOUTH -> new Vector(0, 0, 1);
            case EAST -> new Vector(1, 0, 0);  // bad
    ...
    private static Vector getSecondPoint(BlockFace face) {
        return switch (face) {
            case UP -> new Vector(1, 1, 0);  // bad
            case DOWN -> new Vector(1, 0, 0);
            case NORTH -> new Vector(1, 0, 0);  // bad
            case SOUTH -> new Vector(1, 0, 1);
            case EAST -> new Vector(1, 0, 1);  // bad
    ...
    private static Vector getThirdPoint(BlockFace face) {
        return switch (face) {
            case UP -> new Vector(1, 1, 1);  // bad
            case DOWN -> new Vector(1, 0, 1);
            case NORTH -> new Vector(1, 1, 0);  // bad
            case SOUTH -> new Vector(1, 1, 1);
            case EAST -> new Vector(1, 1, 1);  // bad
